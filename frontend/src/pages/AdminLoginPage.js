...
const AdminFlowAnimation = () => {
  const { bi } = useLang();
  const stats = [
    { label: bi('Активні об’єкти', 'Active assets'), value: '—', Icon: Activity },
    { label: bi('Інвестори', 'Investors'),           value: '—', Icon: Users },
    { label: bi('Залучено', 'Raised'),               value: '—', Icon: TrendingUp },
  ];
    ...
    <div className="w-full max-w-md">
      ...
      >
        <div className="flex items-center gap-2 mb-6">
          ...
          <span className="text-xs ml-2" style={{ color: 'var(--token-text-muted)' }}>{bi('Центр управління', 'Control Center')}</span>
        </div>
        ...
        <div className="mt-6 pt-6" style={{ borderTop: '1px solid var(--token-border)' }}>
          <div className="flex items-center gap-2" style={{ color: 'var(--token-primary)' }}>
            ...
            <span className="text-sm font-medium">{bi('Система справна', 'System healthy')}</span>
          </div>
        </div>
      </div>

      <p className="text-center mt-8 text-sm" style={{ color: 'var(--token-text-muted)' }}>
        {bi('Повний контроль над інвестиційною платформою', 'Full control over your investment platform')}
      </p>
    </div>
  );
};

...
export default function InstitutionalStructure() {
  ...
  useEffect(() => {
    // derive my assets from portfolio
    lumen.get('/institutional/portfolio').then((r) => {
      const hs = r.data.holdings || [];
      setAssets(hs);
      if (hs[0]) setSel(hs[0].asset_id);
    }).catch(() => {});
  }, []);
  ...
  const load = useCallback(async (aid) => {
    ...
    try {
      const [s, g] = await Promise.all([
        lumen.get(`/institutional/assets/${aid}/structure`),
        lumen.get(`/institutional/assets/${aid}/trust-graph`),
      ]);
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 space-y-6" data-testid="institutional-structure">
      <div><div className="text-[11px] uppercase tracking-widest text-muted-foreground">Institutional OS · G1 + G10</div><h1 className="text-2xl font-bold flex items-center gap-2"><Share2 className="w-5 h-5 text-[#2E5D4F]" /> Структура власності</h1><p className="text-sm text-muted-foreground mt-1">Інвестор → Сертифікат → SPV → Оператор → Актив.</p></div>
      <select value={sel} onChange={(e) => setSel(e.target.value)} data-testid="structure-asset-select" className="h-10 rounded-lg border border-border bg-background px-2 text-sm">
        <option value="">— оберіть актив —</option>
        {assets.map((a) => <option key={a.asset_id} value={a.asset_id}>{a.asset_title}</option>)}
      </select>
      {assets.length === 0 && <p className="text-sm text-muted-foreground">У вас немає активів у портфелі.</p>}
      ...
      {!loading && structure && (
        <>
          <div className="grid md:grid-cols-4 gap-3">
            ...
            <Box label="SPV" value={structure.spv?.name} sub={structure.spv?.jurisdiction} />
          ...
          <div><h2 className="font-semibold mb-2">Trust Graph</h2>{graph && <div className="text-foreground"><TrustGraph data={graph} /></div>}</div>
        </>
      )}
    </div>
  );
}
function Box({ label, value, sub, icon, badge }) {
  return <div className="rounded-2xl border border-border bg-card p-4"><div className="text-[11px] uppercase text-muted-foreground flex items-center gap-1.5">{icon}{label}</div><div className="font-semibold mt-1 flex items-center gap-2">{value || '—'}{badge && <VerifiedBadge verified status="verified" />}</div>{sub && <div className="text-[11px] text-muted-foreground">{sub}</div>}</div>;
}

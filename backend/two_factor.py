...
def build_login_router(mobile_user_view) -> APIRouter:
    ...
    async def web_verify(body: ChallengeVerify, request: Request, response: Response):
        ...
        for f in ("password_hash", "totp_secret", "totp_pending_secret", "recovery_codes"):
            user.pop(f, None)
        # F3 · enrich the session created on 2FA-verified login
        try:
            import f3_staff_sessions as _f3
            await _f3.enrich_session_for_user(
                user_id, request, device_fingerprint=getattr(body, "device_fingerprint", None),
            )
        except Exception:
            pass
        return user
